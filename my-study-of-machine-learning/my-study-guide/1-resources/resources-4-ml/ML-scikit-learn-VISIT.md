
1. Andreas C. Müller & Sarah Guido book is the **ULTIMATE BOOK** for scikit learn
1. Building Machine Learning Models in Python with scikit-learn by Janani Ravi
2. [[ON-HOLD] Scikit-learn Tutorial By Vincent D. Warmerdam](https://youtu.be/0B5eIE_1vpU?t=3803)
- https://calmcode.io/course/scikit-learn/introduction
- https://github.com/koaning/calm-notebooks
- RESTART FROM HERE - https://calmcode.io/course/scikit-metrics/metrics
- Make Notes from here - https://youtubetotranscript.com/transcript?v=0B5eIE_1vpU&current_language_code=en
3. [Building Machine Learning Solutions with scikit-learn](https://app.pluralsight.com/paths/skills/building-machine-learning-solutions-with-scikit-learn)
4. [Machine Learning with Python and Scikit-Learn – Full Course By Aakash@Jovi](https://www.youtube.com/watch?v=hDKCxebp88A)
* Lesson 1 - Linear Regression and Gradient Descent
* Lesson 2 - Logistic Regression for Classification
* Lesson 3 - Decision Trees and Random Forests
* Lesson 4 - How to Approach Machine Learning Projects
* Lesson 5 - Gradient Boosting Machines with XGBoost
* Lesson 6 - Unsupervised Learning using Scikit-Learn
* Lesson 7 - Machine Learning Project from Scratch
* Lesson 8 - Deploying a Machine Learning Project with Flask
* https://www.freecodecamp.org/news/machine-learning-with-python-and-scikit-learn/
1. [[_**START-HERE**_] Machine Learning with Python: Zero to GBMs By Aakash@Jovi](https://jovian.com/learn/machine-learning-with-python-zero-to-gbms)
