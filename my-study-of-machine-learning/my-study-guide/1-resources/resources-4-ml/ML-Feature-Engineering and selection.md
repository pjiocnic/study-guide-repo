# Must See

1. [All about Feature Engineering, Feature Store, and Ground Truth in Amazon SageMaker](https://medium.com/@Shamimw/all-about-feature-engineering-feature-store-and-ground-truth-in-amazon-sagemaker-fbdd87cfefec)
2. [What Is the Dummy Variable Trap? (With Pandas Code Examples) Written by Parul Pandey](https://builtin.com/machine-learning/dummy-variable-trap-pandas)


## Documentation

1. [Feature-engine](https://feature-engine.trainindata.com/en/latest/)

## What is FE and How to do it?

1. [Feature Encoding 101: Prepare Data For Machine Learning](https://www.youtube.com/watch?v=kGemHLOEF3w)
2. [Feature Encoding Techniques in Machine Learning with Python Implementation](https://towardsdatascience.com/feature-encoding-techniques-in-machine-learning-with-python-implementation-dbf933e64aa/)

## Categorical Encoding

1. [All about Categorical Variable Encoding by Baijayanta Roy](https://towardsdatascience.com/all-about-categorical-variable-encoding-305f3361fd02/))
2. [What Is the Dummy Variable Trap? (With Pandas Code Examples) Written by Parul Pandey](https://builtin.com/machine-learning/dummy-variable-trap-pandas)
3. [[MUST_SEE] Guide to Encoding Categorical Values in Python by Chris Moffitt](https://pbpython.com/categorical-encoding.html)

## Ordinal Data

[Nominal Data vs. Ordinal Data: What’s the Difference?](https://builtin.com/articles/ordinal-data)

## Nominal vs Ordinal Data

[Nominal Data vs. Ordinal Data: What’s the Difference?](https://builtin.com/articles/ordinal-data)

## Target Encoding

1. [Target-encoding Categorical Variables](https://towardsdatascience.com/dealing-with-categorical-variables-by-using-target-encoder-a0f1733a4c69/)

## Courses

1. [Feature Selection for Machine Learning by Soledad Galli](https://www.udemy.com/course/feature-selection-for-machine-learning)
2. - Learn filter, wrapper, and embedded methods, recursive feature elimination, exhaustive search, feature shuffling & more.
3. Python Feature Engineering Cookbook - Third Edition by Soledad Galli
4. [[EVALUATE] Feature engineering with Python](https://www.trainindata.com/p/feature-engineering-for-machine-learning)
5. [[PURCHASED] Python for Data Analysis](https://justintodata.thinkific.com/courses/take/python-for-data-analysis-projects/lessons/31918251-course-overview)

## Online Material

1. [Feature Engineering and Selection: A Practical Approach for Predictive Models Max Kuhn and Kjell Johnson](http://www.feat.engineering/)


## Videos

1. [Recommending Art with Feature Engineering Sinan Ozdemir](https://learning.oreilly.com/videos/recommending-art-with/10000MNHV202210/10000MNHV202210-aaaaa/)


# SageMaker

1. [[MUST_SEE] All about Feature Engineering, Feature Store, and Ground Truth in Amazon SageMaker](https://medium.com/@Shamimw/all-about-feature-engineering-feature-store-and-ground-truth-in-amazon-sagemaker-fbdd87cfefec)