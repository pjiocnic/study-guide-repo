...
[Watch on YouTube](https://www.youtube.com/watch?v=MUqNwgPjJvQ)
...
[Watch on YouTube](https://www.youtube.com/watch?v=0_4KEb08xrE)
...
<div class="flex justify-center">
    <img scale="50 %" align="center" src="images/local_attention_mask.png"/>
</div>
...
[**Reformer**](https://huggingface.co/docs/transformers/model_doc/reformer) uses **axial positional encodings**.
In traditional Transformer models, the positional encoding $E$ is a matrix of size $l \times d$, where:

* $l$ is the **sequence length**
* $d$ is the **dimension of the hidden state**

If the input sequences are very long, the matrix $E$ can become **huge**, consuming significant GPU memory.

To reduce this, **axial positional encodings** factorize the large matrix $E$ into two smaller matrices $E_1$ and $E_2$, with:

* $E_1 \in \mathbb{R}^{l_1 \times d_1}$
* $E_2 \in \mathbb{R}^{l_2 \times d_2}$
...
The embedding for time step $j$ in $E$ is computed by **concatenating**:

* The embedding for time step $j \bmod l_1$ in $E_1$, and
* The embedding for time step $\left\lfloor \frac{j}{l_1} \right\rfloor$ in $E_2$

This method maintains position-specific information while being much more efficient.
