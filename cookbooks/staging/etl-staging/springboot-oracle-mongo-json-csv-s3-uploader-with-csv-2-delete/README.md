...
### CSV output option
- Set `app.output.format=csv` to write gzip-compressed CSV files instead of JSON.
- The header row is inferred from the first record (keys sorted alphabetically for stability).
- The 3rd `%s` in `app.output.file-name-pattern` becomes `csv`.
