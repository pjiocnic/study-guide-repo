...
### Dry-run (write locally, skip S3)
- Set `app.output.dry-run=true` to bypass S3 and write gzip files to `app.output.local-dir` (default `out/`).
- Example run:
  ```bash
  java -jar target/springboot-oracle-mongo-s3-uploader-0.0.1-SNAPSHOT.jar \
       --spring.config.location=file:./app.properties \
       --app.output.dry-run=true \
       --app.output.local-dir=./out
  ```
