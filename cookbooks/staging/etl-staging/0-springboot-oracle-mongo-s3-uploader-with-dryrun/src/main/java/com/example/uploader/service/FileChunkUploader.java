...
public class FileChunkUploader {
    ...
    public JsonSink newSink(int partNo) throws IOException {
        ...
        // Dry-run: save locally instead of uploading to S3
        if (properties.getOutput().isDryRun()) {
            java.nio.file.Path outDir = java.nio.file.Paths.get(properties.getOutput().getLocalDir());
            java.nio.file.Files.createDirectories(outDir);
            java.nio.file.Path dest = outDir.resolve(filename);
            java.nio.file.Files.move(file.toPath(), dest, java.nio.file.StandardCopyOption.REPLACE_EXISTING);
            log.info("DRY-RUN enabled: wrote {}", dest.toAbsolutePath());
        ...
        if (fmt.equalsIgnoreCase("jsonarray")) {
            ...
            return new CsvGzipSink(temp, properties.getOutput().getCsvColumns());
        } else {
