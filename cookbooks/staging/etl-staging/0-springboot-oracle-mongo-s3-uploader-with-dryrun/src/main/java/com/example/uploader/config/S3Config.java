...
public class S3Config {
    ...
    public S3Client s3Client(AppProperties appProperties) {
        ...
        if (s3Props.getAccessKeyId() != null && !s3Props.getAccessKeyId().isBlank()
            ...
            AwsBasicCredentials creds = (s3Props.getSessionToken() == null || s3Props.getSessionToken().isBlank())
                    ? AwsBasicCredentials.create(s3Props.getAccessKeyId(), s3Props.getSecretAccessKey())
                    : AwsBasicCredentials.create(s3Props.getAccessKeyId(), s3Props.getSecretAccessKey());

            // For session tokens, the builder is different (AwsSessionCredentials), but to keep
            // things simple, rely on environment/instance-role if you need session tokens.
