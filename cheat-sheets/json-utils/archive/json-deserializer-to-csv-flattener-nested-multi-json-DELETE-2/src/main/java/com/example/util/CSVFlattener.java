...
public class CSVFlattener {
    ...
    public Map<String, String> flatten(Person person) {
        ...
        for (String key : fieldToHeaderMap.keySet()) {
            ...
            resolvePath(flatMap, tokens, 0, person, "", headerBase);
        }
        return flatMap;
    }

    private void resolvePath(Map<String, String> flatMap, String[] tokens, int index, Object current, String path, String headerBase) {
        ...
        try {
            ...
            if (isArrayWildcard && value instanceof List<?>) {
                ...
                for (int i = 0; i < list.size(); i++) {
                    ...
                    resolvePathWithSuffix(flatMap, tokens, index + 1, element, newPath, newHeader);
                }
            } else {
                if (index == tokens.length - 1) {
                    ...
                    resolvePath(flatMap, tokens, index + 1, value, newPath, headerBase);
                }
            }
        } catch (Exception e) {
    ...
    private void resolvePathWithSuffix(Map<String, String> flatMap, String[] tokens, int index, Object current, String path, String headerAlias) {
        if (index >= tokens.length || current == null) return;
        ...
        try {
            ...
            if (isArrayWildcard && value instanceof List<?>) {
                ...
                for (int i = 0; i < list.size(); i++) {
                    Object element = list.get(i);
                    String newPath = path + "." + fieldName + "[" + i + "]";
                    String newHeader = headerAlias + "[" + i + "]";
                    resolvePathWithSuffix(flatMap, tokens, index + 1, element, newPath, newHeader);
                }
            } else {
                if (index == tokens.length - 1) {
                    String finalKey = path + "." + token;
                    flatMap.put(finalKey, value != null ? value.toString() : "");
                    actualFields.add(finalKey);
                    actualHeaders.add(headerAlias);
                } else {
                    String newPath = path + "." + fieldName;
                    resolvePathWithSuffix(flatMap, tokens, index + 1, value, newPath, headerAlias);
                }
            }
        } catch (Exception e) {
            String errorKey = path + "." + token;
            flatMap.put(errorKey, "ERROR");
        }
    }
}
